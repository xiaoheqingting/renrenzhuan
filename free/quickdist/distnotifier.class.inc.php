...
class DistNotifier {
  ...
  public static function encode_param($gpc) {
    $params = array(
      '1'=>$gpc['notify_level_1_text'],
      '2'=>$gpc['notify_level_2_text'],
      '3'=>$gpc['notify_level_3_text'],
    );
  ...
  public function create($weid, $gpc) {
    ...
    $ret = pdo_insert(self::$t_notify, array(
      'weid'=>$weid,
      'param'=>$param,
      'level'=>$gpc['level'],
    ));
  ...
  public function update($weid, $gpc) {
    $param = self::encode_param($gpc);
    $ret = pdo_update(self::$t_notify,
      array('param'=>$param, 'level'=>$gpc['level']),
      array('weid'=>$weid));
    return $ret;
  }
        ...
        $msg_template)
  {
    ...
    $map = array(
      ...
      '/\[price\]/'=>$totalprice,
      '/\[return\]/'=>$com_val_acc,
      '/\[credit\]/'=>$com_val_acc * 100,
      '/\[time\]/'=>date('m-d H:i:s', TIMESTAMP));
