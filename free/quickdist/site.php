...
class QuickDistModuleSite extends WeModuleSite {
  ...
  public function doMobileMyCommission() {
    global $_W, $_GPC;
    ...
    $from_user = $_W['openid']; // $_GPC['from_user']; //'oocICs524zTHDJtOmbxJ1ktciNKE';
    $fans = $_fans->get($_W['uniacid'], $from_user);
    // 获取佣金数据
    yload()->classs('quickdist', 'memberorder');
    $_memberorder = new MemberOrder();
    $list = $_memberorder->getCommissionInfo($_W['uniacid'], $from_user);
  ...
  public function doWebMyCommission() {
    ...
    $from_user = $_GPC['from_user']; //'oocICs524zTHDJtOmbxJ1ktciNKE';
    $fans = $_fans->get($_W['uniacid'], $from_user);
    // 获取佣金数据
    yload()->classs('quickdist', 'memberorder');
    $_memberorder = new MemberOrder();
  ...
  public function doWebCommission() {
    ...
    if ($operation == 'display' || $operation == 'detail') {
      ...
      list($list, $total) = $_commission->batchGet($_W['uniacid'], $conds, $pindex, $psize);
      $leaderfans = $_fans->batchGetByOpenids($_W['uniacid'], my_array_fields($list, $com_key), $fans_key);
      $orderfans = $_fans->batchGetByOpenids($_W['uniacid'], my_array_fields($list, $order_fans_key), $fans_key);
    ...
    if ($operation == 'display') {
      ...
      if (checksubmit('submit')) {
        ...
        foreach ($_GPC['orderid'] as $id) {
          ...
          unset($item);
          $_memberorder->giveCommission($_W['uniacid'], $id);
        }
        if ($fail > 0) {
      ...
      list($list, $total) = $_order->batchGet($_W['uniacid'], array('status'=>$status), null, $pindex, $psize);
      $pager = pagination($total, $pindex, $psize);
      if (!empty($list)) {
        foreach ($list as &$row) {
          ...
          // 计算Commission
          $com = $_memberorder->calcCommission($_W['uniacid'], $row['id']);
          foreach ($com as $c) {
            $ids[] = $c['from_user'];
          }
          ...
          if (!empty($com)) {
            $row['commission_info'] =
              '<b>一级返利:</b>' 
              . (empty($com_fans[$com[1]['from_user']]) ? '无上线' : $com_fans[$com[1]['from_user']]['nickname'] . sprintf(' %.2f', $com[1]['com_val']) . '元. ')
              . ' <b>二级返利:</b>' 
              . (empty($com_fans[$com[2]['from_user']]) ? '无上线' :  $com_fans[$com[2]['from_user']]['nickname'] . sprintf(' %.2f', $com[2]['com_val']) . '元. ')
              . ' <b>三级返利:</b>' 
              . (empty($com_fans[$com[3]['from_user']]) ? '无上线' : $com_fans[$com[3]['from_user']]['nickname'] . sprintf(' %.2f', $com[3]['com_val']) . '元. ');
          }
          unset($com_fans);
          unset($ids);
          unset($com);
  ...
  public function doWebNotify() {
    ...
    $notify = $_distnotifier->get($_W['uniacid']);

    if (checksubmit('submit')) {
      if (empty($notify)) {
        $ret = $_distnotifier->create($_W['uniacid'], $_GPC);
      } else {
        $ret = $_distnotifier->update($_W['uniacid'], $_GPC);
      }
  ...
  public function getMemberCountByLevel1() {
    ...
    return "<span  class='label label-warning'>" . $_member->getMemberCountByLevel($_W['uniacid'], $_W['fans']['from_user'], 1, $followonly) . "人</span>";
  }
  ...
  public function getMemberCountByLevel2() {
    ...
    $_member = new member();
    $followonly = intval($this->module['config']['followonly']);
    return "<span  class='label label-warning'>" . $_member->getMemberCountByLevel($_W['uniacid'], $_W['fans']['from_user'], 2, $followonly) . "人</span>";
  }
  ...
  public function getMemberCountByLevel3() {
    ...
    $_member = new member();
    $followonly = intval($this->module['config']['followonly']);
    return "<span  class='label label-warning'>" . $_member->getMemberCountByLevel($_W['uniacid'], $_W['fans']['from_user'], 3, $followonly) . "人</span>";
  }
  ...
  public function getMemberInfoByLevel1() {
    ...
    yload()->classs('quickdist', 'member');
    $_member = new member();
    $mc = $_member->getMemberInfoByLevel($_W['uniacid'], $_W['fans']['from_user'], 1);
  ...
  public function getMemberInfoByLevel2() {
    ...
    yload()->classs('quickdist', 'member');
    $_member = new member();
    $mc = $_member->getMemberInfoByLevel($_W['uniacid'], $_W['fans']['from_user'], 2);
    return $this->genMemberInfoList($mc);
  }
  ...
  public function getMemberInfoByLevel3() {
    ...
    yload()->classs('quickdist', 'member');
    $_member = new member();
    $mc = $_member->getMemberInfoByLevel($_W['uniacid'], $_W['fans']['from_user'], 3);
    return $this->genMemberInfoList($mc);
  }
  ...
  public function getOrderCountByLevel1() {
    ...
    yload()->classs('quickdist', 'memberorder');
    $_memberorder = new MemberOrder();
    $mc = $_memberorder->getOrderCountByLevel($_W['uniacid'], $_W['fans']['from_user'], 1);
    return "<span  class='label label-warning'>" . $_memberorder->getOrderCountByLevel($_W['uniacid'], $_W['fans']['from_user'], 1) . "单</span>";
  }
  ...
  public function getOrderCountByLevel2() {
    ...
    yload()->classs('quickdist', 'memberorder');
    $_memberorder = new MemberOrder();
    return "<span  class='label label-warning'>" . $_memberorder->getOrderCountByLevel($_W['uniacid'], $_W['fans']['from_user'], 2) . "单</span>";
  }
  ...
  public function getOrderCountByLevel3() {
    ...
    yload()->classs('quickdist', 'memberorder');
    $_memberorder = new MemberOrder();
    return "<span  class='label label-warning'>" . $_memberorder->getOrderCountByLevel($_W['uniacid'], $_W['fans']['from_user'], 3) . "单</span>";
  }
  ...
  private function getMyOrderInfoByLevel1($from_user) {
    ...
    yload()->classs('quickdist', 'memberorder');
    $_memberorder = new MemberOrder();
    $mc = $_memberorder->getOrderInfoByLevel($_W['uniacid'], $from_user, 1);
    return $mc;
  }
  ...
  private function getMyOrderInfoByLevel2($from_user) {
    ...
    yload()->classs('quickdist', 'memberorder');
    $_memberorder = new MemberOrder();
    $mc = $_memberorder->getOrderInfoByLevel($_W['uniacid'], $from_user, 2);
    return $mc;
  }
  ...
  private function getMyOrderInfoByLevel3($from_user) {
    ...
    yload()->classs('quickdist', 'memberorder');
    $_memberorder = new MemberOrder();
    $mc = $_memberorder->getOrderInfoByLevel($_W['uniacid'], $from_user, 3);
    return $mc;
  }
  ...
  private function getCommissionInfo() {
    ...
    yload()->classs('quickdist', 'memberorder');
    $_memberorder = new MemberOrder();
    $com_info = $_memberorder->getCommissionInfo($_W['uniacid'], $_W['fans']['from_user']);
    return $com_info;
  }
  ...
  public function getTotalCommission() {
    ...
    foreach ($com_info as $c) {
      if ($c['status'] == Order::$ORDER_NEW
        or $c['status'] == Order::$ORDER_PAYED
        or $c['status'] == Order::$ORDER_DELIVERED
        or $c['status'] == Order::$ORDER_RECEIVED
        or $c['status'] == Order::$ORDER_CONFIRMED) {
          $result += $c['commission'];
        }
    }
