...
$pindex = 1;
$psize = 1024000;
$list = $_request->batchGet($_W['uniacid'], array('status'=>$status), null, $pindex, $psize);
...
$objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('C1', '姓名')
            ->setCellValue('D1', '手机')
            ->setCellValue('E1', '反利方式')
            ->setCellValue('F1', 'OPENID')
            ->setCellValue('G1', '支付宝')
            ->setCellValue('H1', '银行卡号')
            ->setCellValue('I1', '开户行名称')
            ->setCellValue('J1', '款数')
            ->setCellValue('K1', '耗余额')
            ->setCellValue('L1', '备注')
            ->setCellValue('M1', '日期')
...
foreach ($list as $row) {
  $objPHPExcel->setActiveSheetIndex(0)
    ...
    ->setCellValue('E'.$i, $_money->getExchangeTypeStr($row['exchangetype']))
    ->setCellValue('F'.$i, $row['from_user'])
    ->setCellValue('G'.$i, $row['alipay'])
    ->setCellValueExplicit('H'.$i, $row['bankcard'], PHPExcel_Cell_DataType::TYPE_STRING)
    ->setCellValue('I'.$i, $row['bankname'])
    ->setCellValue('J'.$i, $row['cost'])
    ->setCellValue('K'.$i, $row['cost'])
    ->setCellValue('L'.$i, $row['note'])
    ->setCellValue('M'.$i, date('Y-m-d H:i', $row['createtime']))
    ;
  $i++;
  unset($row);
}
