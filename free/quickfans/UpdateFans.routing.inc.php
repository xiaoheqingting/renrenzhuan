...
if (checksubmit('submit')) {
  ...
  $_fans->update($_W['uniacid'], $from_user, $data);
} else if (checksubmit('addcredit1-submit')) {
  $data = intval($_GPC['addcredit1']);
  $_fans->addCredit($_W['uniacid'], $from_user, $data, 1, "管理员后台手动");
  ...
  $_fans->addCredit($_W['uniacid'], $from_user, $data, 2, "管理员后台手动");
  ...
  $_follow = new Follow();
  $_follow->unFollow($_W['uniacid'], $old_leader, $from_user);
} else if (checksubmit('changefollowuplevel')) {
  $old_leader = trim($_GPC['old_leader']);
  $new_leader = trim($_GPC['new_leader']);
  $fans = $_fans->get($_W['uniacid'], $new_leader);
  if (empty($fans)) {
    message('修改失败。您输入的OpenID在系统中不存在, 请检查是否输错了哦。', referer(), 'error');
  }
  yload()->classs('quicklink', 'follow');
  $_follow = new Follow();
  $ret = $_follow->changeFollow($_W['uniacid'], $old_leader, $new_leader, $from_user);
  if (empty($ret)) {
