...
class Auth {
  ...
  private static $author_msg = ""; // "<br>如果还无法解决，请联系作者晓楚(QQ:409975192)<br>";
  ...
  private static $AUTH_COUNT = 1000;


  public function checkXAuth($op, $modulename, $version) {
    //message('授权还没实现呢!', referer(), 'success');
    return true;
    ...
    if (0 == strcasecmp($op, 'auth')) {
      ...
      if (0 == $ret)  {
        ...
        global $_W;
        $re_auth_url = wurl('site/entry/auth', array('m'=>$modulename, 'weid'=>$_W['uniacid'], 'op'=>'Auth'));
        $re_auth_str = "<br><H1><a style='color:red' href='$re_auth_url'>点击这里自助授权</a></H1><br>";
        message($msg . "<br>错误码:{$ret}"
          . "<br><br>" . $re_auth_str
          . $author_msg,
          '',
          'error');
      ...
      // rounting local check
      list($ret, $msg) = $this->checkLocalAuth($modulename, $version);
      if (0 != $ret) {
        global $_W;
        $re_auth_url = create_url('site/entry/auth', array('m'=>$modulename, 'weid'=>$_W['uniacid'], 'op'=>'Auth'));
        $re_auth_str = "<br><H1><a style='color:red' href='$re_auth_url'>点击这里自助授权</a></H1><br>";
        message($msg . "<br>错误码:{$ret}"
          . "<br><br>" . $re_auth_str
          . $author_msg,
  ...
  private function checkRemoteAuth($modulename, $version) {
    $auth_api = 'http://appstore.xiaoheqingting.com/mobile.php?act=module&name=storebackend&do=auth&weid=14'
    ...
    if (is_array($result)) {
      ...
      if (!empty($result['content'])) {
        $data=array('url'=>$result['content']); // 将加密好的url填写到ims_modules中
  ...
  private function getInstalledCount($modulename) {
    // 如果要严格限制，就读取ims_account表，里面包含子公众号
    // 如果要宽松限制，则读取ims_uni_account表，里面只包含主账号
    // 这里，我们暂时采用宽松政策:好处是可以给单个客户更好的体验，和卖更多模块不矛盾
    global $_W;
    $cnt = 0;
