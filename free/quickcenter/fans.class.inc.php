...
class Fans
{
  ...
  public function batchGet($weid, $conds, $key, $pindex, $psize,
    $fields = array('openid as from_user', 'b.nickname', 'avatar', 'credit1', 'follow', 'vip', 'credit2', 'createtime'))
  {
    $fans = array();
    /*
    if (empty($fields)) {
    ...
    $list = pdo_fetchall("SELECT {$fields_str} FROM ".tablename(self::$t_sys_fans)." a, " . tablename('mc_members') . " b WHERE a.uniacid = :uniacid AND a.uid = b.uid {$condition} ORDER BY a.uid DESC "
      . " LIMIT " . ($pindex - 1) * $psize . ',' . $psize,
      array(':uniacid' => $weid), $key);

    $total = pdo_fetchcolumn("SELECT count(*) FROM ".tablename(self::$t_sys_fans)." a, " . tablename('mc_members') . " b WHERE a.uniacid = :uniacid AND a.uid = b.uid {$condition} ",
      array(':uniacid' => $weid));

    return array($list, $total);
  }

  public function batchGetByOpenids($weid, $openids, $key = null,
    $fields = array('from_user', 'nickname', 'avatar', 'credit1', 'follow', 'vip', 'credit2', 'createtime'))
  {
    $fans = array();
    /*
    if (empty($fields)) {
    ...
    */
    if (!empty($openids)) {
      ...
      $fields_str = self::$select_fields;
      $fans = pdo_fetchall("SELECT {$fields_str} FROM " . tablename(self::$t_sys_fans) . " a LEFT JOIN "
        . tablename(self::$t_sys_member) . " b ON a.uid = b.uid WHERE a.openid IN ($openids_str) AND a.uniacid = :uniacid",
          array(':uniacid' => $weid), $key);
    }
    return $fans;
  }
  ...
  public function get($weid, $openid,
    $fields = array('nickname', 'avatar', 'credit1', 'follow', 'vip', 'credit2', 'createtime'))
  {
    return $this->fans_search_by_openid($weid, $openid, $fields);
  }

  public function fans_search_by_openid($weid, $openid,
    $fields = array('nickname', 'avatar', 'credit1', 'follow', 'vip', 'credit2', 'createtime'))
  {
    global $_W;
    $fans = $this->fans_search($openid, $fields);
    if (!empty($fans) and empty($fans['avatar'])) {
      $fans['avatar'] = $_W['siteroot'] . 'addons/quickfans/images/default_head.png';
    }
    return $fans;
  }
  ...
  public function fans_update_by_openid($weid, $openid, $data) {
    ...
    if ($enable_emoji_filter and isset($data['nickname'])) {
      yload()->classs('quickcenter', 'emojiutil');
  ...
  public function getUplevelFans($weid, $follower) {
    global $_W;
    $from_user = pdo_fetchcolumn("SELECT leader FROM " . tablename(self::$t_follow) . " WHERE follower=:follower AND weid=:weid LIMIT 1",
      array(':follower'=>$follower, ':weid'=>$weid));
