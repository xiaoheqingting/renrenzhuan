...
class CreditLog {
  ...
  static $t_credit_log = 'quickcredit_log';

  public function get($weid, $from_user) {
    $ret = pdo_fetchall('SELECT * FROM ' . tablename(self::$t_credit_log) . " WHERE weid=:weid AND from_user=:from_user",
  ...
  public function logCredit($_fans, $weid, $from_user, $credittype, $delta, $tag) {
    ...
    $data = array(
      ...
      'credit' => $fans['credit'.$credittype],
      'delta' => $delta,
      'tag'=>$this->getCreditTypeName($credittype) . '-' . $tag,
