...
class Follow {
  ...
  private function isLeader($weid, $leader) {
    $ret = pdo_fetch("SELECT * FROM " . tablename(self::$t_follow) . " WHERE leader=:leader AND weid=:weid",
      array(":leader"=>$leader, ":weid"=>$weid));
    return $ret;
  }

  private function isFollower($weid, $follower) {
    $ret = pdo_fetch("SELECT * FROM " . tablename(self::$t_follow) . " WHERE follower=:follower AND weid=:weid",
  ...
  public function addFollow($weid, $leader_uid, $follower_uid, $channel) {
    ...
    $ch = pdo_fetch('SELECT sub_click_credit, click_credit, newbie_credit FROM ' . tablename(self::$t_channel) . ' WHERE channel=:channel AND weid=:weid',
      array(':weid'=>$weid, ':channel'=>$channel));
    if (empty($ch)) {
  ...
  public function recordFollow($weid, $leader_uid, $follower_uid, $channel, $click_credit, $sub_click_credit, $newbie_credit) {
    if ($leader_uid != $follower_uid and !empty($leader_uid) and !empty($follower_uid)) {
      ...
      // 如果是首次扫码，扫自己的码，也送一下积分，以后扫别人不会成为别人的下线。
      ...
      // 记录follow关系
      $ret = pdo_insert(self::$t_follow,
        array('weid'=>$weid, 'leader'=>$leader_uid, 'follower'=>$follower_uid, 'channel'=>$channel, 'credit'=>$click_credit, 'createtime'=>time()));
      ...
      $uplevel = pdo_fetch("SELECT * FROM " . tablename(self::$t_follow) . " WHERE weid=:weid AND follower=:follower",
        array(":weid"=>$weid, ":follower"=>$leader_uid));
      if (!empty($uplevel)) {
  ...
  public function isNewUser($weid, $from_user) {
    ...
    $ret = pdo_fetch("SELECT * FROM " . tablename(self::$t_local_fans) . " WHERE  from_user=:from_user AND weid=:weid LIMIT 1",
      array(":from_user"=>$from_user, ":weid"=>$weid));
