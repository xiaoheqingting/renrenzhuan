...
class QuickLinkModuleSite extends WeModuleSite {
  ...
  public function doMobileFollow() {
    global $_W;
    $fans = $this->refreshUserInfo($_W['uniacid'], $_W['fans']['from_user']);
    $mylist = pdo_fetchall("SELECT a.createtime createtime, c.nickname, c.avatar, b.openid from_user FROM " . tablename(self::$t_follow) . " a LEFT JOIN "
      . tablename(self::$t_sys_fans) . " b ON a.follower = b.openid AND a.weid=b.uniacid LEFT JOIN "
      . tablename(self::$t_sys_member) . " c ON b.uid=c.uid "
      . " WHERE leader=:leader AND a.weid=:weid ORDER BY createtime DESC LIMIT 500",
        array(':leader'=>$_W['fans']['from_user'], ':weid'=>$_W['uniacid']));
    $is_follow = true;
  ...
  public function doMobileCredit() {
    ...
    $mylist = pdo_fetchall("SELECT createtime, type, credit FROM " . tablename(self::$t_credit) . " WHERE from_user=:from_user AND weid=:weid ORDER BY createtime DESC",
      array(':from_user'=>$_W['fans']['from_user'], ':weid'=>$_W['uniacid']));
  ...
  public function doMobileTopFollow() {
    ...
    $mylist = $_topfollow->get($_W['uniacid'], $psize);
    $fans = $this->refreshUserInfo($_W['uniacid'], $_W['fans']['from_user']);
  ...
  public function doMobileTopCredit() {
    ...
    $psize = empty($this->module['config']['top_cnt']) ? 10 : $this->module['config']['top_cnt'];
    $mylist = $_topcredit->get($_W['uniacid'], $psize);
    $fans = $this->refreshUserInfo($_W['uniacid'], $_W['fans']['from_user']);
  ...
  public function doWebTopList() {
    ...
    if ($op == 'refresh') {
      ...
      $my_follows_sql = "SELECT a.createtime createtime, nickname, avatar, from_user, COUNT(b.follower) follower_count, SUM(b.credit) follower_credit, credit1 FROM  "
        . tablename('fans') . " a LEFT JOIN " . tablename(self::$t_follow) . " b ON a.from_user = b.leader AND a.weid=b.weid "
        . " WHERE a.weid=:weid GROUP BY a.from_user " . $having . " ORDER BY " . $orderby . " DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize;
      $mylist = pdo_fetchall($my_follows_sql, array(':weid'=>$_W['uniacid']));
      $ret = $_topfollow->updateCache($_W['uniacid'], $mylist);
      if (empty($ret)) {
      ...
      $mylist = $_topfollow->get($_W['uniacid']);
    }
  ...
  public function doWebSpread() {
    ...
    if ($op == 'delete') {
      $ret = $_channel->remove($_W['uniacid'], intval($_GPC['channel']));
      ...
      $mylist = $_channel->batchGet($_W['uniacid']);
      $keywords = array();
      foreach ($mylist as $item) {
        $words = $_channelreply->getKeyword($_W['uniacid'], $item['channel']);
        $keywords[$item['channel']] = $words;
        unset($item);
      }
      $followCount = $_follow->getAllChannelFollowCount($_W['uniacid']);
    } else if ($op == 'active') {
      $channel_id = intval($_GPC['channel']);
      $item = $_channel->getActive($_W['uniacid']);
      $_channel->setActive($_W['uniacid'], $channel_id);
      message('成功！当用户通过分享链接关注本微信号后，会根据本传单设定的积分数给上线送积分', referer(), 'success');
    } else if ($op == 'refresh') {
      // 清除二维码传单缓存
      $_channel->refresh($_W['uniacid'], intval($_GPC['channel']));
      message('清除二维码传单缓存成功', referer(), 'success');
    } else if ($op == 'post') {
      if (checksubmit('submit')) {
       $this->assertIsJPG($_GPC['attachment']);
        if (!empty($_GPC['channel'])) {
          $_channel->update($_W['uniacid'], intval($_GPC['channel']), $_GPC);
          message('更新传单成功', referer(), 'success');
        } else {
          $list_count = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename(self::$t_channel) . " WHERE weid=:weid AND deleted = 0", array(":weid"=>$_W['uniacid']));
          $active = ($list_count == 0);
          $_channel->create($_W['uniacid'], $active, $_GPC);
          message('新建传单成功', $this->createWebUrl('spread', array('op'=>'leaflet')), 'success');
        }
      }
      $item = array();
      if (!empty($_GPC['channel'])) {
        $item = $_channel->get($_W['uniacid'],$_GPC['channel']);
      } else {
        $item = $_channel->decode_channel_param($item, null); // fill item with default value
      }
      ...
      $my_follows_sql = "SELECT t_member.createtime createtime, t_member.nickname, avatar, t_follow.from_user, t_member.credit1, "
        . "                      t_follow.follower_count, t_follow.follower_credit "
        . " FROM (SELECT COUNT(b.follower) follower_count, SUM(b.credit) follower_credit, b.leader from_user, weid, leader FROM " . tablename(self::$t_follow) . " b "
        . " WHERE weid={$_W['uniacid']} {$where} GROUP BY b.leader {$having} ORDER BY {$orderby} DESC LIMIT {$poffset}, {$psize}) t_follow "
        . " LEFT JOIN " . tablename(self::$t_sys_fans) . " t_fans ON t_follow.weid = t_fans.uniacid AND t_follow.leader = t_fans.openid "
        . " LEFT JOIN " . tablename(self::$t_sys_member) . " t_member ON t_fans.uniacid = t_member.uniacid AND t_fans.uid = t_member.uid";

      $my_follows_size_sql = "SELECT COUNT(follower) follower_count FROM " . tablename(self::$t_follow)  . " b WHERE weid={$_W['uniacid']} {$where} GROUP BY b.leader {$having}";
      ...
      $uplevel = pdo_fetch("SELECT * FROM " . tablename(self::$t_follow) . " WHERE weid=:weid AND follower=:follower",
        array(":weid"=>$_W['uniacid'], ":follower"=>$from_user));
      $fans = fans_search($from_user, array('avatar', 'nickname', 'createtime', 'credit1'));
      $mylist = pdo_fetchall("SELECT a.createtime createtime, c.nickname, avatar, openid from_user, credit FROM " . tablename(self::$t_follow) . " a LEFT JOIN "
        . tablename(self::$t_sys_fans) . " b ON a.follower = b.openid AND a.weid=b.uniacid LEFT JOIN "
        . tablename(self::$t_sys_member) . " c ON b.uniacid = c.uniacid AND b.uid = c.uid  "
        . " WHERE leader=:leader AND a.weid=:weid",
          array(':leader'=>$from_user, ':weid'=>$_W['uniacid']));
      ...
      if (count($mylist) > 0) {
        //  $mylist2 = pdo_fetchall("SELECT * FROM "
        //    . "(SELECT * FROM " . tablename(self::$t_follow) . " a WHERE  a.leader in (" . $from_user_list . ") AND a.weid=:weid ) b LEFT JOIN "
        //    . tablename('fans') . " c ON b.follower = c.from_user AND b.weid=c.weid WHERE b.weid=:weid2",
        //    array(':weid'=>$_W['uniacid'], ':weid2'=>$_W['uniacid']));

        $mylist2 = pdo_fetchall("SELECT * FROM " . tablename(self::$t_follow) . " a LEFT JOIN "
          . tablename(self::$t_sys_fans) . " b ON a.follower = b.openid AND a.weid=b.uniacid LEFT JOIN "
          . tablename(self::$t_sys_member) . " c ON b.uniacid = c.uniacid AND b.uid = c.uid  "
          . " WHERE  a.leader in (" . $from_user_list . ") AND a.weid=:weid",
            array(':weid'=>$_W['uniacid']));
      }

    } else if ('black_remove' == $op) {
        $from_user = $_GPC['from_user'];
        pdo_delete(self::$t_black, array('from_user'=>$from_user, 'weid'=>$_W['uniacid']));
        message('执行成功', $this->createWebUrl('Spread', array('op'=>'black')), 'success');
    } else if ('black' == $op) {
      if (!empty($_GPC['from_user'])) {
        $from_user = $_GPC['from_user'];
        $b = pdo_fetch("SELECT * FROM " . tablename(self::$t_black) . " WHERE from_user=:f AND weid=:w LIMIT 1", array(':f'=>$from_user, ':w'=>$_W['uniacid']));
        if (empty($b)) {
          pdo_insert(self::$t_black, array('from_user'=>$from_user, 'weid'=>$_W['uniacid'], 'access_time'=>time()));
        }
        message('添加黑名单成功', referer(), 'success');
      }
      $list =  pdo_fetchall("SELECT * FROM " . tablename(self::$t_black) . " WHERE weid=:w", array(':w'=>$_W['uniacid']));
    } else {
      message('error!', '', 'error');
    }
  ...
  public function doWebRanking() {
    ...
    if ($op == 'log' or $op == 'qualitylog') {
      ...
      $uplevel = pdo_fetch("SELECT * FROM " . tablename(self::$t_follow) . " WHERE weid=:weid AND follower=:follower",
        array(":weid"=>$_W['uniacid'], ":follower"=>$from_user));
      $mylist = pdo_fetchall("SELECT a.createtime createtime, c.nickname, avatar, a.follower from_user, credit FROM " . tablename(self::$t_follow) . " a LEFT JOIN "
        . tablename(self::$t_sys_fans) . " b ON a.follower = b.openid AND a.weid=b.uniacid LEFT JOIN "
        . tablename(self::$t_sys_member) . " c ON b.uniacid = c.uniacid AND b.uid = c.uid  "
        . " WHERE leader=:leader AND a.weid=:weid ORDER by a.createtime DESC",
          array(':leader'=>$from_user, ':weid'=>$_W['uniacid']));
      $from_user_list = "'xxxooo'";
      foreach($mylist as $l) {
        $from_user_list .= ",'" . $l['from_user'] . "'";
      ...
      if (count($mylist) > 0) {
        $mylist2 = pdo_fetchall("SELECT a.createtime createtime, c.nickname, c.avatar, a.leader, a.follower openid FROM " . tablename(self::$t_follow) . " a LEFT JOIN "
          . tablename(self::$t_sys_fans) . " b ON a.follower = b.openid AND a.weid=b.uniacid LEFT JOIN "
          . tablename(self::$t_sys_member) . " c ON b.uniacid = c.uniacid AND b.uid = c.uid  "
          . " WHERE  a.leader in (" . $from_user_list . ") AND a.weid=:weid ORDER BY a.createtime DESC",
            array(':weid'=>$_W['uniacid']));
      }
    } else {
      message('error!', '', 'error');
  ...
  public function doWebBlackList() {
    ...
    if ('black_remove' == $op) {
      ...
      $list =  pdo_fetchall("SELECT * FROM " . tablename(self::$t_black) . " WHERE weid=:w ORDER BY access_time DESC", array(':w'=>$_W['uniacid']));
    } else {
      message('error!', '', 'error');
    }
  ...
  private function clearQRCache($ch)
  {
    ...
    //  array(':weid'=>$_W['uniacid'], ':channel'=>$ch));
  }
  ...
  public function doWebUnlink() {
    ...
    yload()->classs('quicklink', 'follow');
    $_follow = new Follow();
    $_follow->unFollow($_W['uniacid'], $_GPC['leader'], $_GPC['follower']);
  ...
  public function doMobileRunTask() {
    ...
    $_responser->respondText($_W['uniacid'], $_GPC['from_user'], intval($_GPC['channel_id']), $_GPC['rule']);
    exit(0);
  }
  ...
  private function link($u) {
   ...
   public function doMobileClearQR() {
    global $_W;
    $ret = pdo_fetchall("SELECT * FROM " . tablename(self::$t_qr) . " WHERE weid={$_W['uniacid']} LIMIT 100");
    print_r($ret);
    $ret = pdo_fetchall("SELECT * FROM " . tablename(self::$t_channel) . " WHERE weid={$_W['uniacid']}");
    print_r($ret);
    ...
    echo "<br>";
    echo "<br>";
    $ret = pdo_fetchall("SELECT * FROM " . tablename(self::$t_qr) . " WHERE weid={$_W['uniacid']}");
    print_r($ret);
