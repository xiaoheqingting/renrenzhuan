...
class Channel {
  ...
  public function get($weid, $channel_id) {
    $ret = pdo_fetch("SELECT * FROM " . tablename(self::$t_channel) . " WHERE weid=:weid AND channel=:channel",
      array(":weid"=>$weid, ":channel"=>$channel_id));
  ...
  public function create($weid, $active, $gpc) {
    ...
    pdo_insert(self::$t_channel,
      ...
      'desc'=>$gpc['desc'],
      'url'=>$gpc['url'],
      'click_credit'=>$gpc['click_credit'],
      'sub_click_credit'=>$gpc['sub_click_credit'],
      'newbie_credit'=>$gpc['newbie_credit'],
      'weid'=>$weid));
    return $ret;
  }
  ...
  public function update($weid, $channel_id, $gpc) {
    ...
    $ret = pdo_update(self::$t_channel,
      array(
        ...
        'sub_click_credit'=>$gpc['sub_click_credit'],
        'newbie_credit'=>$gpc['newbie_credit']),
      array('channel'=>$channel_id, 'weid'=>$weid));
    return $ret;
  }
  ...
  public static function decode_channel_param(&$item, $p) {
    ...
    $item['notify_not_leader_scan_text'] = $gpc['notify_not_leader_scan_text'];
    // 平滑升级,去掉原来的图片上传方式
    if (!empty($item['bg'])) {
  ...
  public static function encode_channel_param($gpc) {
    $params = array(
      'qrquality'=>intval($gpc['qrquality']),
      ...
      'namecolor'=>$gpc['namecolor'],
      'nameenable'=>intval($gpc['nameenable']),
      'genqr_info1' => $gpc['genqr_info1'],
      'genqr_info2' => $gpc['genqr_info2'],
      'genqr_info3' => $gpc['genqr_info3'],
      'genqr_vip_limit_info' => $gpc['genqr_vip_limit_info'],
      'vip_limit' => $gpc['vip_limit'],
      'bgimages' => self::encode_image_array($gpc),
      'notify_leader_follow_text'=>$gpc['notify_leader_follow_text'],
      'notify_uplevel_follow_text'=>$gpc['notify_uplevel_follow_text'],
      'notify_leader_scan_text'=>$gpc['notify_leader_scan_text'],
      'notify_not_leader_scan_text'=>$gpc['notify_not_leader_scan_text'], /* 已经关注过，扫其他人的二维码时提示 */
    );
