...
class OrderSearch {
  ...
  public function verifyById($weid, $orderid, $cond) {
    ...
    $order = pdo_fetch("SELECT * FROM " . tablename(self::$t_order)
      . " WHERE weid = :weid AND id = :orderid LIMIT 1", array(':weid'=>$weid, ':orderid'=>$orderid));
  ...
  public function verify($weid, $conds) {
    ...
    $list = pdo_fetchall("SELECT * FROM " . tablename(self::$t_order)
    ...
    if (!empty($addressids)) {
      yload()->classs('quickshop', 'address');
      $_address = new Address();
      $addresses = $_address->batchGetByIds($_W['uniacid'], $addressids, 'id');
    }
  ...
  public function search($weid, $conds = array(), $key = null, $pindex, $psize) {
    ...
    if (!empty($conds['expresssn'])) {
      $condition .=" AND expresssn = '" . $conds['expresssn'] . "' ";
    }
    ...
    foreach ($list as $row) {
      ...
      } else if (isset($conds['expresssn'])) {
        if (FALSE !== strpos($row['expresssn'], $conds['expresssn'], 0))  $orders[] = $row;
      }
    }
