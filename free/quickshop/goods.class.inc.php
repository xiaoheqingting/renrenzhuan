...
class Goods {
  ...
  public static $PHYSICAL_GOODS =  1;  //实体物品
  public static $VIRTUAL_GOODS =  2;  // 虚拟物品
  ...
  public function swapParent($weid, $id, $pgoodsid) {
    if (!empty($pgoodsid) && !empty($id)) {
      // 先把id的pgoodsid修改为0
      pdo_update(self::$t_goods, array("pgoodsid" => 0), array('id' => $id, 'weid'=> $weid));
      // 然后把parentid的pgoodsid修改为id
      pdo_update(self::$t_goods, array("pgoodsid" => $id), array('id' => $pgoodsid, 'weid'=> $weid));
      // 然后把旧的转向新主人
      pdo_update(self::$t_goods, array("pgoodsid" => $id), array('pgoodsid' => $pgoodsid, 'weid'=> $weid));
    }
  }
  ...
  public function hasBuy($weid, $from_user, $goodsid) {
    $flag = pdo_fetch("SELECT a.id FROM " . tablename(self::$t_order) . ' a LEFT JOIN '
      . tablename(self::$t_order_goods) . ' b ON a.id = b.orderid AND a.weid=b.weid
      WHERE a.from_user=:from_user AND a.weid=:weid AND b.goodsid=:goodsid AND b.weid=:weid AND a.status >= 3 AND a.status <= 6 LIMIT 1',
      array(':from_user'=>$from_user, ':goodsid'=>$goodsid, ':weid'=>$weid));
    return $flag;
  }


  // 出于性能考虑，专门写一个update可以少一次查询
  ...
  public function batchGet($weid, $conds = array(), $pindex = 1, $psize = 100000, $key = null) {
    ...
    $list = pdo_fetchall("SELECT * FROM " . tablename(self::$t_goods)
      . " WHERE weid = $weid and deleted=0 $condition ORDER BY status DESC, pcate DESC, displayorder DESC, id DESC "
  ...
  public function batchGetBySecondaryCategory($weid, $pcate, $ccate, $other_cond, $pindex = 1, $psize = 100000) {
    $conds = array_merge(array('cate_1'=>$pcate, 'cate_2'=>$ccate, 'status'=>1), $other_cond);
    return $this->batchGet($weid, $conds, $pindex, $psize);
  }
  ...
  public function batchGetByIds($weid, $ids, $key = null) {
    $goods = array();
    if (!empty($ids) and count($ids) > 0 and is_array($ids)) { // prevent sql error
      $goods = pdo_fetchall("SELECT * FROM " . tablename(self::$t_goods) . " WHERE id IN ('" . implode("','", $ids) . "')",
        array(),
        $key);
    }
    return $goods;
