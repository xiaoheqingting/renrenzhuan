...
//list($list, $total) = $_order->batchGet($_W['uniacid'], array('status'=>$status, 'allstatus'=>!isset($status), 'sendtype'=>$sendtype), null, $pindex, $psize);
list($list, $total) = $_order->batchGet($_W['uniacid'], array('status'=>$status, 'allstatus'=>!isset($status)), null, $pindex, $psize);
if (!empty($list)) {
  foreach ($list as &$row) {
...
$objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue($cmap['id'].'1', 'ID')
            ->setCellValue($cmap['ordersn'].'1', '订单号')
            ->setCellValue($cmap['status'].'1', '状态')
            ->setCellValue($cmap['nickname'].'1', '昵称')
            ->setCellValue($cmap['realname'].'1', '姓名')
            ->setCellValue($cmap['mobile'].'1', '手机')
            ->setCellValue($cmap['openid'].'1', 'OPENID')
            ->setCellValue($cmap['address'].'1', '快递地址')
            ->setCellValue($cmap['price'].'1', '总价')
            ->setCellValue($cmap['createtime'].'1', '下单时间')
            ->setCellValue($cmap['detail'].'1', '订单详情')
            ->setCellValue($cmap['remark'].'1', '备注')
            ->setCellValue($cmap['paytype'].'1', '付款方式')
            ->setCellValue($cmap['sendtype'].'1', '快递类型')
            ;

$i = 2;
foreach ($list as $listrow) {
  $objPHPExcel->setActiveSheetIndex(0)
    ->setCellValue($cmap['id'].$i, $listrow['id'])
    ->setCellValueExplicit($cmap['ordersn'].$i, $listrow['ordersn'], PHPExcel_Cell_DataType::TYPE_STRING)
    ->setCellValue($cmap['status'].$i, $listrow['status'])
    ->setCellValue($cmap['nickname'].$i, $listrow['nickname'])
    ->setCellValue($cmap['realname'].$i, $listrow['realname'])
    ->setCellValueExplicit($cmap['mobile'].$i, $listrow['mobile'], PHPExcel_Cell_DataType::TYPE_STRING)
    ->setCellValue($cmap['openid'].$i, $listrow['from_user'])
    ->setCellValue($cmap['address'].$i, $listrow['address'])
    ->setCellValue($cmap['price'].$i, $listrow['price'])
    ->setCellValue($cmap['createtime'].$i, date('Y-m-d H:i', $listrow['createtime']))
    ->setCellValue($cmap['detail'].$i, $listrow['goods'])
    ->setCellValue($cmap['remark'].$i, $listrow['remark'])
    ->setCellValue($cmap['paytype'].$i, $listrow['paytype'])
    ->setCellValue($cmap['sendtype'].$i, $listrow['sendtype'])
    ;
  $i++;
  unset($listrow);
}
