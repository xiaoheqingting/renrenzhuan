<?php
/* @brief A class for you to inherit, easy to get openid
 * class interface:
...
class OpenAuth extends WeModuleSite 
{
  ...
  private function getAccount() {
    ...
    $account = $_W['account'];
    if(empty($account)){
      $wechat = pdo_fetch("SELECT * FROM ".tablename('wechats')
    ...
    if(!empty($code)) {
      ...
      if(!is_error($ret)) {
        ...
        if(is_array($auth) && !empty($auth['openid'])) {
          ...
          header('location: ' . $_W['siteroot'] . 'mobile.php?' . $forward 
            .'&' . $this->cookiename . '='. $auth['openid']
