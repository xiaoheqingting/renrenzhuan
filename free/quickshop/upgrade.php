...
CREATE TABLE if not exists `ims_quickshop_link` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `weid` int(11) default 0,
  `title` varchar(50) DEFAULT '' COMMENT '链接显示文字',
...
if(!pdo_fieldexists('quickshop_goods', 'permusers')) {
	pdo_query("ALTER TABLE ".tablename('quickshop_goods')." ADD `permusers` varchar(100);");
	pdo_query("ALTER TABLE ".tablename('quickshop_goods')." ADD `lastedituser` varchar(100);");
	pdo_query("ALTER TABLE ".tablename('quickshop_order')." ADD `lastorderedituser` varchar(1000) NOT NULL DEFAULT '';");
}
...
if(pdo_fieldexists('quickshop_adv', 'link')) {
	pdo_query("ALTER TABLE ".tablename('quickshop_adv')." CHANGE `link` `link` varchar(1024) NOT NULL DEFAULT '';");
}

if(pdo_fieldexists('quickshop_adv', 'advname')) {
	pdo_query("ALTER TABLE ".tablename('quickshop_adv')." CHANGE `advname` `advname` varchar(255) NOT NULL DEFAULT '';");
}


if(!pdo_fieldexists('quickshop_adv', 'type')) {
  pdo_query("ALTER TABLE ".tablename('quickshop_adv')." ADD `type` int(11) DEFAULT 0 COMMENT '0表示顶部幻灯片,1表示中部大推荐图'");
}
...
global $_W;
//更新原有规格
$goods = pdo_fetchall("select id from ".tablename('quickshop_goods')." where weid=:weid",array(":weid"=>$_W['uniacid']));
$optionids = array();
foreach($goods as $o){
...
//交换成本价和市场价位置
$goods = pdo_fetchall("select id,costprice,productprice from ".tablename('quickshop_goods')." where weid=:weid",array(":weid"=>$_W['uniacid']));
foreach($goods as $o){
...
if(!pdo_fieldexists('quickshop_goods', 'promote_to_level')) {
	pdo_query("ALTER TABLE ".tablename('quickshop_goods')." ADD `promote_to_level` int(10) default 0 COMMENT '最低显示级别，低于这个级别的用户看不到该商品';");
}
...
if(!pdo_fieldexists('quickshop_goods', 'use_abs_commission')) {
	pdo_query("ALTER TABLE ".tablename('quickshop_goods')." ADD `use_abs_commission` tinyint(2) default 0 COMMENT '为1表示使用绝对值返利,为0表示百分比';");
}
