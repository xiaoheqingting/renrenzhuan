...
class Address {
  ...
  public function clientGet($weid, $openid, $id) {
    $address = pdo_fetch('SELECT * FROM ' . tablename(self::$t_address) . ' WHERE id=:id AND weid=:weid AND openid=:openid LIMIT 1',
      array(':id'=>$id, ':weid'=>$weid, ':openid'=>$openid));
    return $address;
  }

  public function find($data) {
    $address = pdo_fetch('SELECT * FROM ' . tablename(self::$t_address) . ' WHERE weid=:weid AND openid=:openid ' 
      . ' AND realname=:realname AND mobile=:mobile AND province=:province AND ' 
      . ' city=:city AND area=:area AND address=:address LIMIT 1',
      array(
        ':weid'=>$data['weid'],
        ':openid'=>$data['openid'],
  ...
  public function getDefault($weid, $openid, $usesys=true) {
    ...
    if (empty($address) && true == $usesys) {
      // set admin defined default address
  ...
  public function markDelete($weid, $openid, $id, $isdefault = false) {
    ...
    if ($isdefault) { // select new default address
  ...
  public function batchGet($weid, $conds) {
    ...
    $list = pdo_fetchall("SELECT * FROM " . tablename(self::$t_address) 
      . " WHERE weid = $weid and deleted=0 $condition ORDER BY isdefault DESC, id DESC ");
