...
class OrderNotifier {
  ...
  // 本函数让quickshop依赖了quicklink，后期要考虑解除这种依赖。
    ...
    if (1) {
      ...
      if (!empty($setting['autoreply_rid'])) {
        ...
        if (!empty($channel_reply)) {
          ...
          $url = WechatUtil::createMobileUrl('RunTask', 'quicklink',
            array('from_user'=>$order['from_user'], 'channel_id'=>$channel_reply['channel'], 'rule'=> $channel_reply['content']));
    ...
    if (!empty($template_id)) {
      $data = array(
        ...
        'data'=> array(
          ...
          'OrderSn'=>array('value'=>$order['ordersn'], 'color'=>'#173177'),
          'OrderStatus'=>array('value'=>$_order->getOrderStatusName($order['status'], $order['goodstype']), 'color'=>'#173177'),
  ...
  public function notifyCancel($param) {
    ...
    if (!empty($template_id)) {
      $data = array(
        ...
        'topcolor'=>'#ff0000',
        'data'=> array(
          'first'=> array('value'=>'订单已取消', 'color'=>'#173177'),
          'OrderSn'=>array('value'=>$order['ordersn'], 'color'=>'#173177'),
          'OrderStatus'=>array('value'=>$_order->getOrderStatusName($order['status'], $order['goodstype']), 'color'=>'#173177'),
          'remark'=>array('value'=>'您好，您的订单已被后台小二取消，感谢您的支持。', 'color'=>'#173177')
        ),
      );
      $_weapi->sendTemplateMsg($data);
    } else {
